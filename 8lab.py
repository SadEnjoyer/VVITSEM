...
elf._update_day_table(self.monday1_table, day, week)
    ...
    def _change_day_from_table(self, table, rowNum, day, week):
        ...
        for i in range(day_table.columnCount()):
            try:
                ...
                row.append(None)
        self.cursor.execute(
            f"UPDATE timetable SET subject='{row[0]}', start_time='{row[1]}', room='{row[2]}' WHERE id_days='{day}_{week}' AND id='{records[rowNum][0]}'")
        self.conn.commit()
    ...
    def _add_class_(self, table, rowNum, day, week):
        ...
        for i in range(day_table.columnCount()):
            try:
                ...
                row.append(None)
        self.cursor.execute(
            f"INSERT INTO timetable (id,id_days, subject,start_time, room) VALUES ('{id}','{day}_{week}','{row[0]}','{row[1]}', '{row[2]}')")
        self.conn.commit()
    ...
    def _add_to_teacher_list(self, rowNum):
        ...
        for i in range(self.teacher_table.columnCount()):
            try:
                ...
                row.append(None)
        self.cursor.execute(
            f"INSERT INTO teacher (id_teacher, full_name, subject) VALUES ('{id_teacher}','{row[0]}','{row[1]}')")
        self.conn.commit()
        id_teacher += 1
